...
void provision_wifi(bool ap){
const char SSID[] = "Testf";
const char PW[] = "12345678";
const char * network = SSID;
const char * pw = PW;
if(ap){
    wifi_cmd_ap_set(network, pw);
}
else{
    wifi_cmd_sta_join(network, pw);    
}
    
}

void wifi_scan(void){
    wifi_cmd_sta_scan("Testf");
}
...
void iperf(bool ap){
    ...
    cfg.sport = IPERF_DEFAULT_PORT;
    cfg.dport = IPERF_DEFAULT_PORT;
    cfg.interval = 1;//IPERF_DEFAULT_INTERVAL;
    cfg.time = 10; //IPERF_DEFAULT_TIME;

    }
    else{ // creates a iperf client
        cfg.flag |= IPERF_FLAG_CLIENT;
        cfg.destination_ip4 = esp_ip4addr_aton("192.168.1.12");
        cfg.source_ip4 = wifi_get_local_ip();
        cfg.flag |= IPERF_FLAG_TCP;
        cfg.len_send_buf = 0;
        cfg.sport = IPERF_DEFAULT_PORT;
